...
import NewItemForGroupPopUp from "../components/NewItemForGroupPopUp";
import NewItemForUserPopUp from "../components/NewItemForUserPopUp";
...
export default function GroupView() {
  ...
  const closeGroupPopUp = () => {
    setNewGroupItemPopUp(false);
  };

  const closeUserPopUp = () => {
    setNewUserItemPopUp(false);
  };
  ...
  const getTotalConsumed = (userId) => {
    ...
    user.consumedItems.forEach((item) => {
      totalValue +=
        (getConsumedItemData(item.id).price /
          getConsumedItemData(item.id).totalQuantity) *
        item.quantity;
    });
  ...
  return (
    <View style={styles.container}>
        ...
        {group.users.map((user, index) => (
          <View key={index} style={styles.userContainer}>
            <Text style={styles.username}>
              {findUserByCPF(user.cpf).username}
            </Text>
            {user.consumedItems.map((consumedItem, idx) => (
              <View key={idx} style={styles.itemContainer}>
                <Text style={styles.itemName}>
                  {getConsumedItemData(consumedItem.id).name}:{" "}
                  {consumedItem.quantity}
                </Text>
                <Text style={styles.itemPrice}>
                  R${" "}
                  {(
                    (getConsumedItemData(consumedItem.id).price /
                      getConsumedItemData(consumedItem.id).totalQuantity) *
                    consumedItem.quantity
                  ).toFixed(2)}
                </Text>
              </View>
            ...
            <Text style={styles.totalItems}>
              Total: R$ {getTotalConsumed(user.cpf)}
            </Text>
      ...
      <NewItemForGroupPopUp
        visible={newGroupItemPopUp}
        onClose={closeGroupPopUp}
      />
      <NewItemForUserPopUp
        visible={newUserItemPopUp}
